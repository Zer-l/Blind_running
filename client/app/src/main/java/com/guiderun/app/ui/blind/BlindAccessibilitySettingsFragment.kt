...
class BlindAccessibilitySettingsFragment : Fragment() {
    ...
    private val viewModel: BlindAccessibilitySettingsViewModel by viewModels()
        ...
        binding.seekBarTtsSpeed.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    ...
                    binding.tvTtsSpeedValue.text = String.format(java.util.Locale.ROOT, "%.1fx", speed)
                }
            }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    binding.tvTtsSpeedValue.text = String.format(java.util.Locale.ROOT, "%.1fx", state.ttsSpeed)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.events.collect { event ->
                    if (event == BlindAccessibilitySettingsEvent.RecreateRequired) {
