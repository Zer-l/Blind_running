...
class BlindWaitingMatchFragment : Fragment() {
    ...
    private val viewModel: BlindWaitingMatchViewModel by viewModels()
    ...
    private suspend fun collectNavEvents() {
        viewModel.navEvent.collect { event ->
            when (event) {
                is BlindWaitingMatchNavEvent.ToMatched -> {
                ...
                is BlindWaitingMatchNavEvent.ToHome -> {
    ...
    override fun onResume() {
        super.onResume()
        // 接力 TTS：上游（BlindMatchedViewModel MATCHING 分支）通过 setPendingWaitingTts 暂存原因。
