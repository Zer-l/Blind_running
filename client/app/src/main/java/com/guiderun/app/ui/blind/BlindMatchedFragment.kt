...
class BlindMatchedFragment : Fragment() {
    ...
    private val viewModel: BlindMatchedViewModel by viewModels()
    ...
    private suspend fun collectNavEvents() {
        viewModel.navEvent.collect { event ->
            when (event) {
                is BlindMatchedNavEvent.ToHome -> {
                ...
                is BlindMatchedNavEvent.ToWaitingMatch -> {
                    // 接力 TTS：让 BlindWaitingMatchFragment.onResume 内消费并 speak HIGH
                    event.reasonRes?.let {
                        (activity as? BlindActivity)?.setPendingWaitingTts(it)
                    }
                    val nc = findNavController()
                    // 优先 pop 回栈中已有的 BlindWaitingMatchFragment，复用其 elapsed 计时；
                ...
                is BlindMatchedNavEvent.ToRunning -> {
                    val args = Bundle().apply { putString("requestId", event.requestId) }
