...
| 版本 | 文件 | 说明 |
|------|------|------|
| Release APK | [app-release.apk](apk/app-release.apk)（约 53 MB） | R8 混淆 + 资源压缩 |

> **运行环境要求**：Android 8.0（API 26）及以上，推荐 Android 10+

运行前需在 `client/local.properties` 配置高德地图 Key 和讯飞 AppId（详见[快速开始](#快速开始)），或联系作者获取预配置版本。
...
| 首页 | 发起请求 | 跑步中 | 无障碍设置 |
|------|---------|--------|-----------|
| ![首页](screenshots/blind_home.jpg) | ![发起请求](screenshots/blind_create.jpg) | ![跑步中](screenshots/blind_running.jpg) | ![无障碍设置](screenshots/blind_a11y.jpg) |

### 志愿者端

| 接单列表 | 接单详情 | 跑步中 | 轨迹回放 |
|---------|---------|--------|---------|
| ![接单列表](screenshots/volunteer_orders.jpg) | ![接单详情](screenshots/volunteer_detail.jpg) | ![跑步中](screenshots/volunteer_running.jpg) | ![轨迹回放](screenshots/volunteer_playback.jpg) |
...
- 震动反馈（tick / confirm / warning / error 四档语义化）
...
```kotlin
haptic.tick()       // 轻触反馈（短按）
haptic.confirm()    // 长按阈值/操作确认（重振）
haptic.warning()    // 警告/撤销（长振）
haptic.error()      // 操作失败（三短振）
```
...
PATCH  /api/v1/users/me                 # 更新基础信息（含 roles 字段，首次设置触发 PENDING_ROLE → ACTIVE）
GET    /api/v1/users/me/blind-stats     # 视障端跑步统计
GET    /api/v1/users/me/volunteer-stats # 志愿者端跑步统计
GET    /api/v1/users/{userId}/reviews   # 查询某用户收到的评价（分页）
```

### 紧急联系人接口

```http
GET    /api/v1/users/me/blind-profile/emergency-contacts          # 列表
POST   /api/v1/users/me/blind-profile/emergency-contacts          # 新增
PATCH  /api/v1/users/me/blind-profile/emergency-contacts/{index}  # 更新（按列表下标定位）
DELETE /api/v1/users/me/blind-profile/emergency-contacts/{index}  # 删除
```
...
```http
# 查询
POST   /api/v1/run-requests                       # 发起请求（视障端，支持 Idempotency-Key）
GET    /api/v1/run-requests                       # 当前用户历史列表（按 role 分端，支持 status 多值筛选）
GET    /api/v1/run-requests/active                # 当前活跃订单（冷启动恢复用）
GET    /api/v1/run-requests/available             # 附近可接单（志愿者用，支持 radius）
GET    /api/v1/run-requests/{id}                  # 订单详情

# 状态推进
POST   /api/v1/run-requests/{id}/accept           # 志愿者接单
POST   /api/v1/run-requests/{id}/depart           # 志愿者确认出发
POST   /api/v1/run-requests/{id}/confirm-met      # 确认汇合（双端均可）
POST   /api/v1/run-requests/{id}/start-run        # 开始跑步
POST   /api/v1/run-requests/{id}/request-end-run  # 志愿者申请结束跑步（推送通知，不改状态）
POST   /api/v1/run-requests/{id}/end-run          # 视障端最终确认结束 → FINISHED

# 终止
POST   /api/v1/run-requests/{id}/cancel           # 视障端取消（唯一退出路径）
POST   /api/v1/run-requests/{id}/abandon          # 志愿者放弃接单（连续 3 次降级为 ABORTED）
POST   /api/v1/run-requests/{id}/emergency        # 紧急求助 → ABORTED

# 实时上报与轨迹/评价
POST   /api/v1/run-requests/{id}/position         # 志愿者位置上报（WebSocket 之外的兜底通道）
POST   /api/v1/run-requests/{id}/peer-metrics     # 推送跑步实时指标
POST   /api/v1/run-requests/{id}/tracks           # 上传轨迹
GET    /api/v1/run-requests/{id}/tracks           # 查询轨迹
POST   /api/v1/run-requests/{id}/reviews          # 提交评价
...
// 服务端 → 客户端
{ "type": "status_changed", "requestId": "...", "toStatus": "MET", "version": 3, "triggeredRole": "BLIND", "isReminder": false }
...
- Release APK ≈ 53 MB（含讯飞 .so + 高德 SDK），全 pipeline 通过 `minifyReleaseWithR8`
